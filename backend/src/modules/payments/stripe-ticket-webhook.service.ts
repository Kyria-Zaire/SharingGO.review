...
function parseTicketCheckoutMetadata(session: Stripe.Checkout.Session): StripeCheckoutMetadata {
...
export async function handleTicketCheckoutSessionCompleted(
  event: Stripe.Event,
  ...
  const metadata = parseTicketCheckoutMetadata(session);
  const now = new Date();
