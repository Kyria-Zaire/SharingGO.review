...
import { PROFILE_HERO_CONTENT } from "@/features/profile/constants/profile-content";
import {
...
export function ProfileHeroSection({ onLogout }: { onLogout: () => void }) {
      ...
      <div className={cn(landingContainerClass, "relative z-10")}>
        <div className={cn(heroShellClass, "max-lg:pb-14")}>
          ...
          <div className="relative">
            <h1 id="profile-hero-title" className={heroTitleClass}>
              {PROFILE_HERO_CONTENT.titleBefore}
              <span className="text-primary">{PROFILE_HERO_CONTENT.titleHighlight}</span>
            </h1>

            <p className={heroSubtitleClass}>{PROFILE_HERO_CONTENT.subtitle}</p>
            ...
            >
              ...
              {PROFILE_HERO_CONTENT.logoutCta}
            </button>
          </div>
        </div>
      </div>
